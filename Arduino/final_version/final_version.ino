...
void setup() {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    ...
    timeout -= 1;
    if(timeout == 0){
      Serial.println("Time out connection wifi");
      ESP.restart();
    }
  }
...
void loop(){
  ...
  if(currentHour == HEURE_AROSAGE and !wattered and ARROSAGE_AUTOMATIQUE == 1){
  ...
  //capture image
  if(TIMELAPSE == 1)compteurTimelapse++;
  if( (TIMELAPSE == 1 && compteurTimelapse >= FREQUENCE_TIMELAPSE) || FORCE_PHOTO == 1){
